...
class Context(object):
	...
	def make_noun_phrase(self, case, noun_type, has_adj=-1, compound_nums=-1):
            ...
            def _make_simple_noun_phrase(case, noun_type, has_adj=-1, adj_nums=-1,\
                                            for_conjunct=0, eliminated_words=eliminated_words):
                ...
                while word_entry.nom() in eliminated_words:
                    word_entry = choice(noun_vocab_list)
                eliminated_words.append(word_entry.nom())
            ...
            #enddef _make_simple_noun_phrase

            if compound_nums == -1:
                    compound_nums = weighted_choice([85,15,5]) + 1
            result, number = _make_simple_noun_phrase(case, noun_type, has_adj=has_adj)
            for i in range(compound_nums-1):
                    new_simple_noun_phrase, _ = _make_simple_noun_phrase(case, noun_type, adj_nums=1, for_conjunct=1)
